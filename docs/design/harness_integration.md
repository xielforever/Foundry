...
| `agent_id` | `string` | 否 | 目标 Agent ID（为空时使用自动发现模式） |
...
| `on_failure` | `string` | 否 | 失败策略（OnFailureStrategy 枚举值：human_intervention/auto_rollback/retry_then_intervention/retry_then_rollback/skip，默认 human_intervention） |
...
> **设计决策**：Harness failure strategy 统一设为 `MarkAsFailure`（不重试），原因：1) 重试由 Foundry 内部管理，Harness 不理解 Agent 语义；2) Harness 重试会重新创建 Step 容器，导致 Foundry 侧状态不一致；3) Foundry 的 on_failure 策略（OnFailureStrategy 枚举：human_intervention/auto_rollback/retry_then_intervention/retry_then_rollback/skip）在 Foundry Core 内部执行，不需要 Harness 参与。
...
import "foundry/v1/intervention.proto";
...
message SubmitTaskRequest {
  ...
  string stage_id = 3;
  string step_id = 4;
  OnFailureStrategy on_failure = 5;
  int32 retry_limit = 6;
  string agent_id = 7;
...
| v1.2 | 2026-05-06 | 跨文档同步修正：1) SubmitTaskRequest.on_failure 字段类型从 string 改为 OnFailureStrategy 枚举（与 Task 5 一致）；2) 新增 import intervention.proto；3) Plugin 输入参数 on_failure 可选值更新为 OnFailureStrategy 枚举值；4) 配置示例 on_failure 值从 intervene 改为 human_intervention | Foundry Team |
| v1.3 | 2026-05-06 | 一致性审查修正：1) SubmitTaskRequest 新增 timeout_seconds 字段（field 8）；2) agent_id 标记为非必填（为空时使用自动发现模式）；3) max_storage_mb 默认值从 512 更新为 1024（与 Task 6 一致） | Foundry Team |
