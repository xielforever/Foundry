...
| 问题 | 解决任务 | 说明 |
|------|----------|------|
| Artifact Store 存储选型 | Task 6 | 审计持久化方案确定后，Artifact Store 可复用或独立选型 |
| Artifact 大小限制 | Task 3 | Executor 实现时确定各 Agent 类型的 Artifact 大小上限 |
| 自定义 Artifact 类型的 Schema 注册机制 | Task 4 | Agent 注册时可同时注册自定义 Artifact 类型的 Schema |
| Task 调度结果记录 | Task 3 | Task 数据模型未包含 `assigned_agent_id` 字段，调度到哪个具体 Agent 实例由 Scheduler 运行时管理，是否需要在 Task 中记录调度结果待 Task 3 确定 |
| Artifact 产出数量约束 | Task 3 | `expected_artifact_types` 未定义每种类型的数量约束（如"必须产出恰好 1 个 REPORT"），是否需要增加数量声明待 Task 3 确定 |
