...
│   │   ├── common.proto          # 共享枚举（AgentType/ArtifactType/ContentFormat/ExecutionStatus）
│   │   ├── executor.proto        # Executor 接口定义
│   │   ├── scheduler.proto       # Scheduler 服务定义
│   │   ├── task.proto            # Task 消息定义
│   │   ├── artifact.proto        # Artifact 消息定义
│   │   ├── intervention.proto    # 失败处理枚举（InterventionStatus/RollbackGranularity/OnFailureStrategy）
│   │   ├── registry.proto        # Registry 服务定义
│   │   ├── audit.proto           # Audit 服务与事件定义
...
1. **v1 存储方案**：默认使用嵌入式 SQLite（审计日志、Artifact Store、Intervention 记录），可选 PostgreSQL（生产环境），详见 audit_scheme.md
2. **v1 不引入 Web UI**：通过 CLI 交互，Web UI 留作 v2
3. **Go 版本锁定 1.22+**：利用泛型和标准库 Slog

## 待决问题

- [x] 审计持久化存储选型（文件 / SQLite / 其他）——在 Task 6 中决定 ✅ 已解决：SQLite 默认 + PostgreSQL 可选，详见 audit_scheme.md
- [x] Foundry CLI 与 Harness 的具体通信方式（CLI 调用 vs HTTP API vs 两者兼有）——在 Task 7 中决定 ✅ 已解决：gRPC（Plugin → Foundry Core）+ HTTP JSON（外部 API），详见 harness_integration.md
- [x] Agent 容器化执行的网络模式（host / bridge / none）——在 Task 3 中决定 ✅ 已解决：bridge 网络模式，详见 agent_executor_architecture.md
...
| v1.3 | 2026-05-06 | 跨文档同步修正：1) 待决问题 3 项标记为已解决（审计存储选型/通信方式/网络模式）；2) 约束"v1 不引入数据库"更新为"默认 SQLite + 可选 PostgreSQL"；3) proto/ 目录新增 common.proto、intervention.proto，更新 audit.proto 说明 | Foundry Team |
