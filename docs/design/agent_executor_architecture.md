...
Scheduler 在接收到 Task 后，按以下优先级选择 Executor：
...
| 编号 | 问题 | 需要解决的任务 | 说明 |
|------|------|-------------|------|
| OQ-3.1 | Executor 是否支持自定义启动前脚本（initContainer） | Task 7 | 某些 Agent 需在任务执行前运行初始化脚本；容器化部署时可使用 Docker initContainer |
| OQ-3.2 | gRPC Execute 的负载均衡策略 | Task 4 | 同类型多个 Executor 实例时，Scheduler 按什么策略选择（轮询/最少连接/加权） |
| OQ-3.3 | Artifact 产出后是否自动触发下游 Step | Task 7 | Harness Step 的完成由 Harness 管理，Foundry 不主动触发 |
...
| OQ-3.5 | Human Gate 的 Web UI 形态 | Task 7 | v1 的 Human Gate 通过 Web Hook 回调，是否需要 Foundry 提供审批 UI 还是依赖外部审批系统 |
